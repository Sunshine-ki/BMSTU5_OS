...
; 16МБ - это доступная физическая память в эмуляторе dos-box
; GDT - системная таблица.
; базовый вектор прерываний - смещение в табл. дескр. прер.
...
descr struct	; Начало объялвения структуры. (Описание структуры != выделение памяти).
    ...
    ; (base_1) 16 + (base_m) 8 + (base_h) 8 = 32 (2^30 (Гбайт) + 2^2 = 4Гбайт)
...
; Данный дескриптор назвывается шлюзом.
idescr struc
...
data segment use16 ; use16 - это тип. в данном сегменте будут использоваться 16-битовые адреса.
	...
	gdt_data descr 		<data_size-1, 0, 0, 10010010b, 0, 0>  ; 92h
	gdt_code descr 		<code_size-1, 0, 0, 10011000b, 0, 0>  ; 98h - задаем attr_1 (сегмент команд)
    ; этот код (98h) говорит о том, что исполняемый сегмент, к которому запрещено обращение с целью записи и чтения.
	gdt_stack descr 	<255, 0, 0, 10010010b, 0, 0>          ; 92h - задаем attr_1 (сегмент данных (или стека) с кодом 
    ...
    gdt_screen descr 	<3999, 8000h, 0Bh, 10010010b, 0, 0>   ; attr_2 - там указан тип дробности в нашем случае он везде равен 0 (т.е. граница указывается в байтах)
...
continue:
	...
	LOOP print2 ; LOOP Выполняется до тех пор, пока CX не обнулится 
	...
	; Если мы просто перейдем в р-р сбросом бито в регистре CR0
	...
	; Переключим режим процессора в р-р.
go:  
	MOV EAX, CR0
